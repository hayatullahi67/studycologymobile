...
//             // If AI Speech is running, stop it and reset its state
...
//                 if (status.isLoaded) {
//                     status.isPlaying ? await soundRef.current.pauseAsync() : await soundRef.current.playAsync();
//                 }
//                 return;
//             }
...
//             const { sound: newSound } = await Audio.Sound.createAsync({ uri: audioUri }, { shouldPlay: true, progressUpdateIntervalMillis: 100 }, onPlaybackStatusUpdate);
...
//         if (!trimmedText) {
//             // If manual audio is playing, stop it
//             if (soundRef.current) {
//                 soundRef.current.stopAsync();
...
//                         <View style={[styles.detailAudioContainer, { backgroundColor: C.primaryLight, borderColor: C.primaryBorder, opacity: audioLoading === activeNote?.id ? 0.7 : 1 }]}>
//                             <TouchableOpacity 
//                                 style={[styles.detailPlayBtn, { backgroundColor: C.brown }]}
//                                 onPress={() => playAudio(activeNote.audio_url, activeNote.id)}
//                                 disabled={!!audioLoading}
//                             >
//                                 {audioLoading === activeNote?.id ? (
//                                     <ActivityIndicator size="small" color="#FFF" />
//                                 ) : (
//                                     <Ionicons name={activeAudioId === activeNote?.id && playbackStatus.isPlaying ? "pause" : "play"} size={20} color="#FFF" />
//                                 )}
//                             </TouchableOpacity>
//                             <View style={{ flex: 1 }}>
//                                 <Text style={styles.detailAudioText}>Admin Voice Note</Text>
//                                 <View style={styles.detailProgressBarBg}>
//                                         <View style={[styles.detailProgressBarFill, { width: activeAudioId === activeNote.id ? `${(playbackStatus.position / playbackStatus.duration) * 100}%` : '0%' }]} />
//                                 </View>
//                                 {activeAudioId === activeNote?.id && (
...
//     speechBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderStyle: 'dashed' },
//     speechBtnText: { fontSize: 13, fontWeight: '700' },
//     detailAudioContainer: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: 16, borderWidth: 1 },
...
export function NoteDetailScreen() {
    ...
    // ── Pause the admin audio player (without unloading) ──────────────────────
    ...
    // ── Pause the speech player (without resetting progress) ─────────────────
    ...
    const playAudio = async (url: string, id: string) => {
        try {
            ...
            // ── If speech is playing or paused, pause it first ────────────────
            if (speechStatus === 'playing') {
    ...
    const speakFromPosition = async (fullText: string, startIndex = 0, rate = speechRate) => {
        ...
        // ── If admin audio is playing, pause it first ─────────────────────────
