...
type NoteQuizRouteProp = RouteProp<RootStackParamList, 'NoteQuiz'>;

export function NoteQuizScreen() {
    const route = useRoute<NoteQuizRouteProp>();
    ...
    const loadNote = async () => {
        try {
            setLoading(true);
            const data = await localDB.getNoteByIdLocal(noteId);
            if (data && data.quiz && data.quiz.length > 0) {
                setNote(data);
            } else {
                Alert.alert("Error", "No quiz available for this note.");
                navigation.goBack();
            }
        } catch (error) {
    ...
    const currentQuestion = note.quiz[currentQuestionIndex];
    ...
    const nextQuestion = () => {
        if (currentQuestionIndex < note.quiz.length - 1) {
            setCurrentQuestionIndex(currentQuestionIndex + 1);
            setSelectedAnswer(null);
            setShowExplanation(false);
        } else {
            setQuizFinished(true);
            localDB.saveNoteQuizResult(noteId, score, note.quiz.length);
        }
    };
            ...
            <Screen style={[styles.bg, { backgroundColor: '#FFF8F6' }]}>
                ...
                <View style={styles.resultContainer}>
                    <View style={[styles.resultCircle, { borderColor: '#3E2723', backgroundColor: 'rgba(62, 39, 35, 0.05)' }]}>
                        ...
                        <Text style={[styles.scoreText, { color: '#3E2723' }]}>{score} / {note.quiz.length}</Text>
                        <Text style={[styles.scoreLabel, { color: '#5D4037' }]}>CORRECT ANSWERS</Text>
                    </View>

                    <Text style={[styles.congratsText, { color: '#1E293B' }]}>
                        {score === note.quiz.length ? "Perfect Score!" : score > note.quiz.length / 2 ? "Well Done!" : "Keep Studying!"}
                    </Text>
                    ...
                    >
                        <Text style={styles.finishBtnText}>Go Back to Note</Text>
                    </TouchableOpacity>
                </View>
            </Screen>
        );
        ...
        <Screen scrollable={false} style={[styles.bg, { backgroundColor: '#FFF8F6' }]}>
            ...
            <View style={styles.progressContainer}>
                <View style={styles.progressBar}>
                    <View
                        style={[
                            styles.progressFill,
                            {
                                width: `${((currentQuestionIndex + 1) / note.quiz.length) * 100}%`,
                ...
                <Text style={styles.progressText}>
                    QUESTION {currentQuestionIndex + 1} OF {note.quiz.length}
                </Text>
            </View>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                <View style={[styles.instrCard, { backgroundColor: '#EFEBE9', borderColor: '#D7CCC8' }]}>
                    ...
                    <Text style={[styles.instrText, { color: '#5D4037' }]}>Choose the correct answer based on the study note.</Text>
                </View>
            ...
            <View style={[styles.footer, { backgroundColor: '#FFF8F6', borderTopColor: '#E2E8F0' }]}>
                ...
                >
                    <Text style={[styles.nextBtnText, { color: selectedAnswer ? '#FFF' : '#A1887F' }]}>
                        {currentQuestionIndex < note.quiz.length - 1 ? "NEXT QUESTION" : "FINISH QUIZ"}
                    </Text>
