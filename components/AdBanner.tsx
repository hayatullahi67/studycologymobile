...
interface AdBannerProps {
    ...
    mockAd?: any;
    carousel?: boolean;
    size?: 'small' | 'medium' | 'large' | 'option' | 'square';
}
...
export function AdBanner(props: AdBannerProps) {
    ...
    const handleAdClick = async () => {
        ...
        // Only track clicks for real ads, not previews
        if (!props.mockAd && currentAd.id) {
            try {
                await supabaseDB.trackAdClick(currentAd.id);
            } catch (e) {
        ...
        const rawLink = currentAd.link_url || currentAd.linkUrl;
        if (rawLink) {
            try {
    ...
    const hasLink = !!(currentAd.link_url || currentAd.linkUrl);
        ...
        return (
            ...
            >
                <View style={styles.optionContent}>
                    ...
                    {hasLink && <Ionicons name="chevron-forward" size={16} color="#94A3B8" />}
                </View>
            </TouchableOpacity>
        );
    }
        ...
        >
            ...
            <View style={[styles.content, isSmall && { padding: 8, paddingVertical: 10 }]}>
                ...
                {hasLink && (
                    <View style={[styles.ctaButton, isSmall && { paddingVertical: 6, paddingHorizontal: 10, marginTop: 4 }]}>
                        <Text style={[styles.ctaText, isSmall && { fontSize: 12 }]}>
                            {currentAd.button_text || currentAd.buttonText || 'Learn More'}
                        </Text>
                        <Ionicons
                            name="arrow-forward"
                            size={isSmall ? 12 : 16}
