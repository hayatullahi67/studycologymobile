 ...
 * A robust Rich Text Editor for React Native using Quill.js in a WebView.
 * Supports font sizes, font families, and all standard formatting.
 * Scrolling is handled internally by the WebView.
 */
export function RichTextEditor({
    initialValue = '',
    onChange,
...
<head>
    ...
    <style>
        * {
        ...
        /* Toolbar stays sticky at top while content scrolls */
        .ql-toolbar.ql-snow {
            border: none;
            border-bottom: 1px solid ${isDark ? 'rgba(230, 81, 0, 0.1)' : '#e2e8f0'};
            background-color: ${isDark ? '#0D0D0D' : '#f8fafc'};
        ...
        /* Editor grows with content — WebView scrolls natively */
        #editor {
            min-height: ${height - 65}px;
            height: auto;
            border: none;
            font-size: 16px;
        }
    ...
    return (
        <View style={[styles.container, { height, backgroundColor: isDark ? '#050505' : '#ffffff' }]}>
            <WebView
                ...
                onMessage={handleMessage}
                onLoadEnd={syncInitialValue}
                scrollEnabled={true}          // ← WebView handles all scrolling internally
                nestedScrollEnabled={true}    // ← Needed on Android for scroll inside parent
                javaScriptEnabled={true}
                domStorageEnabled={true}
                style={{ flex: 1, backgroundColor: 'transparent' }}
                renderLoading={() => (
